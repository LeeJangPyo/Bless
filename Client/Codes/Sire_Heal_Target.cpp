...
CSire_Heal_Target::CSire_Heal_Target()
	:m_pInput_Device(CInput_Device::GetInstance())
{
	m_pInput_Device->AddRef();
}

HRESULT CSire_Heal_Target::Begin()
{
...
_int CSire_Heal_Target::Update(const _float & fTimeDelta)
{
...
_int CSire_Heal_Target::LastUpdate(const _float & fTimeDelta)
{
...
HRESULT CSire_Heal_Target::End()
{
	m_pEffect_TargetZone->Set_Activate(false);
	return S_OK;
}

void CSire_Heal_Target::Check_KeyInput(const _float & fTimeDelta)
{
...
HRESULT CSire_Heal_Target::Ready_State(CSire* pSire)
{
	m_pSire = pSire;
	...
	for (auto& pGameObject : *pGameObjectList)
	{
		...
		if (pUIFrame == nullptr)
			return E_FAIL;
		if (pUIFrame->Compare_Tag(L"Skill_Heal"))
		{
			m_pUIFrame = pUIFrame;
			break;
		}
...
CSire_Heal_Target * CSire_Heal_Target::Create(CSire* pSire)
{
	CSire_Heal_Target*		pInstance = new CSire_Heal_Target();

	if (FAILED(pInstance->Ready_State(pSire)))
	{
		_MSG_BOX(L"CSire_Heal_Target Clone Failed");
...
void CSire_Heal_Target::Free()
{
	Safe_Release(m_pInput_Device);
	Safe_Release(m_pCalculator);
}

_vec3 CSire_Heal_Target::PickUp_OnTerrain(void)
{
