...
CTerrain_Stage2::CTerrain_Stage2(LPDIRECT3DDEVICE9 pDevice)
	:CGameObject(pDevice)
...
CTerrain_Stage2::CTerrain_Stage2(const CTerrain_Stage2 & rhs)
	:CGameObject(rhs)
	, m_pTransformCom(nullptr)
	, m_pTextureCom(nullptr)
	, m_pNormalTextureCom(nullptr)
...
HRESULT CTerrain_Stage2::Ready_Prototype()
{
	return S_OK;
}

HRESULT CTerrain_Stage2::Ready_GameObject()
{
...
_int CTerrain_Stage2::Update_GameObject(const _float & fTimeDelta)
{
	return _int();
}

_int CTerrain_Stage2::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
void CTerrain_Stage2::Render_GameObject()
{
...
HRESULT CTerrain_Stage2::Ready_Component()
{
	...
	if (FAILED(Add_Component(L"Com_Navi", m_pNaviMeshCom)))
		return E_FAIL;
	m_pNaviMeshCom->Load_InfoFromFile(L"../Bin/Resources/Data/stage2.navi");

	// For. Stage Component
	m_pBufferCom = dynamic_cast<CBuffer_TerrainTex*>(pManagement->Clone_Component(SCENE_STAGE, L"Component_Buffer_Terrain_Stage2"));
...
HRESULT CTerrain_Stage2::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (pEffect == nullptr)
		return E_FAIL;
	...
	if (FAILED(m_pFilterTextureCom->SetUp_OnShader(pEffect, "g_FilterTexture",1)))
		return E_FAIL;
...
CTerrain_Stage2 * CTerrain_Stage2::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CTerrain_Stage2*		pInstance = new CTerrain_Stage2(pDevice);
...
CGameObject * CTerrain_Stage2::Clone_GameObject()
{
	CTerrain_Stage2*		pInstance = new CTerrain_Stage2(*this);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX(L"CTerrain_Stage2 Clone Failed");
		Safe_Release(pInstance);
	}
