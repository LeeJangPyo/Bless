...
void Client::CCamera_Dynamic::Check_InputKey(const _float& fTimeDelta)
{
	...
	if (m_pInput_Device->Get_KeyPressing(DIK_NUMPAD8))
	{
		_vec3		vLook;
	...
	if (m_pInput_Device->Get_KeyPressing(DIK_NUMPAD5))
	{
		_vec3		vLook;
		memcpy(&vLook, &matCamWorld.m[2][0], sizeof(_vec3));
	...
	if (m_pInput_Device->Get_KeyPressing(DIK_NUMPAD4))
	{
		_vec3		vRight;
	...
	if (m_pInput_Device->Get_KeyPressing(DIK_NUMPAD6))
	{
		_vec3		vRight;
		memcpy(&vRight, &matCamWorld.m[0][0], sizeof(_vec3));
	...
	if (m_pInput_Device->Get_KeyPressing(DIK_NUMPAD7))
	{
		_vec3		vUp;
	...
	if (m_pInput_Device->Get_KeyPressing(DIK_NUMPAD9))
	{
		_vec3		vUp;
		memcpy(&vUp, &matCamWorld.m[1][0], sizeof(_vec3));
