...
void CUI_Resources_Loader::UI_Resources_Load(const wstring& wstrFilePath)
{
	...
	while (iContinue != -1)
	{
		...
		if (FindData.size != 0)
		{
			...
			CManagement::GetInstance()->Add_Prototype_Component(SCENE_STAGE, wstrComponentTag.c_str(),
...
HRESULT CUI_Resources_Loader::Effect_Mesh_Load(const wstring & wstrFilePath)
{
	...
	while (iContinue != -1)
	{
		...
		if (FindData.size != 0)
		{
			...
			if (FAILED(CManagement::GetInstance()->Add_Prototype_Component(SCENE_STAGE, wstrComponentTag.c_str(),
