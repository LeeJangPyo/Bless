...
void CEffect_Particle::SetUp_ParticleInfo(_int iParicleNum)
{
	...
	for (_int i = 0; i < m_ParticleData.iParticleNum; ++i)
	{
		...
		m_pParticle[i].iAnimationX = 0;
		m_pParticle[i].iAnimationY = 0;
		m_pParticle[i].fFrame = 0.f;
	}
}



HRESULT CEffect_Particle::SetUp_ConstantTable(LPD3DXEFFECT pEffect, const  _int& iIndex)
{
	if (nullptr == pEffect)
		return E_FAIL;
...
//	}
	...
	if (FAILED(m_pParticle[iIndex].pTransformCom->SetUp_OnShader_BillBoardMatrix(pEffect, "g_matWorld")))
			return E_FAIL;
...
void CEffect_Particle::Save_EffectData(HANDLE & hFile, _ulong & dwByte)
{
	...
	//EffectParticle 데이터
	WriteFile(hFile, &m_ParticleData, sizeof(EFFECTPARTICLE_DATA), &dwByte, nullptr);

	WriteFile(hFile, &m_bRecycle, sizeof(_bool), &dwByte, nullptr);
}

void CEffect_Particle::Load_EffectDataFromFile(HANDLE & hFile, _ulong & dwByte, CEffectTab* pEffectTab)
{
	//Effect 기본 데이터
	ReadFile(hFile, &m_EffectData, sizeof(EFFECTDATA), &dwByte, nullptr);

	//EffectParticle 데이터
	ReadFile(hFile, &m_ParticleData, sizeof(EFFECTPARTICLE_DATA), &dwByte, nullptr);

	ReadFile(hFile, &m_bRecycle, sizeof(_bool), &dwByte, nullptr);
...
void CEffect_Particle::Set_Position(const _vec3 * pPosition)
{
	m_pTransformCom->Set_StateInfo(CTransform::STATE_POSITION, pPosition);

	for (_int i = 0; i < m_ParticleData.iParticleNum; ++i)
	{
		m_pParticle[i].vInital_Position = *pPosition;
	}
}
...
void CEffect_Particle::Set_ParticleNum(_int iParticleNum)
{
	if (iParticleNum == m_ParticleData.iParticleNum)
		return;
	...
	SetUp_ParticleInfo(iParticleNum);
...
HRESULT CEffect_Particle::Ready_Effect()
{
	...
	m_ParticleData.fLifeTime_Range = 50.f;
	...
	if(!m_bRecycle)
	{
		...
		if (m_fTimeAcc > m_EffectData.fStartTime)
		{
			...
			for (_int i = 0; i < m_ParticleData.iParticleNum; ++i)
			{
				...
				pParticle->iAnimationY = (_int)pParticle->fFrame / m_EffectData.iAlpha_Width;
...
void CEffect_Particle::Render_Effect(LPD3DXEFFECT pEffect)
{
	if (pEffect == nullptr)
		return;
