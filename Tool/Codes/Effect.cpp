...
HRESULT CEffect::SetUp_AlphaTexutre(const _tchar * pComponentTextureTag)
{
	if (m_pAlphaTextureCom != nullptr)
	{
		...
		ZeroMemory(m_EffectData.szAlphaTextureTag, sizeof(_tchar)*MIN_PATH);
	}
	...
	//For. Texture;
	m_pAlphaTextureCom = dynamic_cast<CTexture*>(CManagement::GetInstance()->Clone_Component(SCENE_EDIT, pComponentTextureTag));
	if (m_pAlphaTextureCom == nullptr)
		return E_FAIL;

//	m_pAlphaTextureCom = dynamic_cast<CTexture*>(CManagement::GetInstance()->Clone_Component(SCENE_EDIT, L"Component_Texture_Explosion"));
...
HRESULT CEffect::SetUp_BaseTexture(const _tchar * pComponentTextureTag)
{
	...
	if (m_pBaseTextureCom != nullptr)
	{
		...
		ZeroMemory(m_EffectData.szBaseTextureTag, sizeof(_tchar)*MIN_PATH);
	}
	...
	//For. Texture;
	m_pBaseTextureCom = dynamic_cast<CTexture*>(CManagement::GetInstance()->Clone_Component(SCENE_EDIT, pComponentTextureTag));
	if (m_pBaseTextureCom == nullptr)
		return E_FAIL;
...
HRESULT CEffect::SetUp_NoiseTexture(const _tchar * pComponentTextureTag)
{
	if (m_pNoiseTextureCom != nullptr)
	{
		...
		ZeroMemory(m_EffectData.szNoiseTextureTag, sizeof(_tchar)*MIN_PATH);
	}
	...
	//For. Texture;
	m_pNoiseTextureCom = dynamic_cast<CTexture*>(CManagement::GetInstance()->Clone_Component(SCENE_EDIT, pComponentTextureTag));
	if (m_pNoiseTextureCom == nullptr)
		return E_FAIL;
...
HRESULT CEffect::Ready_Effect()
{
	...
	m_EffectData.vStart_Scale = _vec3(1.f,1.f,1.f);
	m_EffectData.vEnd_Scale = _vec3(1.f, 1.f, 1.f);

	m_EffectData.fEndTime = 1.f;

	m_EffectData.iAlpha_Width = 1;
	m_EffectData.iAlpha_Height = 1;
	m_EffectData.iBase_Width = 1;
	m_EffectData.iBase_Height = 1;
	m_EffectData.fAlpha = 1.f;
...
void CEffect::Render_Effect(LPD3DXEFFECT pEffect)
{
...
HRESULT CEffect::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	...
	//pEffect->SetFloat("g_fGradation", 0.3f);
