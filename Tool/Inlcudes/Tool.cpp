...
BOOL CToolApp::OnIdle(LONG lCount)
{
	...
	else
	{
		...
		if (m_pFrame_Manager->Permit_Call(L"Frame_60", fTimeDelta_Default) == true)
		{
			_float		fTime_Delta_60 = m_pTimer_Manager->Get_TimeDelta(L"Timer_60");

			m_pMainApp->Update_MainApp(fTime_Delta_60);
...
BOOL CToolApp::InitApplication()
{
	...
	// For.Timer_Default
	if (FAILED(m_pTimer_Manager->Add_Timer(L"Timer_Default")))
		return FALSE;

	// For.Timer_60
	if (FAILED(m_pTimer_Manager->Add_Timer(L"Timer_60")))
		return FALSE;

	// For.Frame_60
	if (FAILED(m_pFrame_Manager->Add_Frame(L"Frame_60", 60)))
		return FALSE;
